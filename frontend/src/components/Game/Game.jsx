...
function Game(props) {
  ...
  const pressHandler = (event) => {
      if(event.keyCode === 101) socket.emit('catch_spell');
  }
  ...
  if(room.gameStage === STAGE_SEEKER_WIN) {
    return (
      <div className={classes.blackScreen}>
        <h1 className={classes.redTitle}>SEEKERS WIN!</h1>
      </div>
    )
  }
  ...
  if(room.gameStage === STAGE_HIDERS_WIN) {
    return (
      <div className={classes.blackScreen}>
        <h1 className={classes.greenTitle}>HIDERS WIN!</h1>
      </div>
    )
  }
  ...
  return (
    <div className={classes.canvasContainer}>
      ...
      {room.gameStage === STAGE_PLAYING ? <Timer gameStage={room.gameStage} timer={room.playingTimer} color='black'/> : null}
        <Canvas flat linear>
            <color attach="background" args={['lightblue']} color='black'/>
            <OrbitControls 
